document.addEventListener('DOMContentLoaded', () => {
    ...
    const perfilIdeal = {
        "Equilibrio": { 
            ...
            soluciones: {
                ...
                gasBajo: (val) => `Gas Bajo: Tu ajuste de <strong>${val} bar</strong> está por debajo del rango recomendado (${perfilIdeal.Equilibrio.gasRecomendado.min}-${perfilIdeal.Equilibrio.gasRecomendado.max} bar) para esta fase.`,
                gasAlto: (val) => `Gas Alto: Tu ajuste de <strong>${val} bar</strong> está por encima del rango recomendado (${perfilIdeal.Equilibrio.gasRecomendado.min}-${perfilIdeal.Equilibrio.gasRecomendado.max} bar) para esta fase.`,
        ...
        "Amarillo": { 
            ...
            soluciones: {
                ...
                gasBajo: (val) => `Gas Bajo: Tu ajuste de <strong>${val} bar</strong> está por debajo del rango recomendado (${perfilIdeal.Amarillo.gasRecomendado.min}-${perfilIdeal.Amarillo.gasRecomendado.max} bar) para esta fase.`,
                gasAlto: (val) => `Gas Alto: Tu ajuste de <strong>${val} bar</strong> está por encima del rango recomendado (${perfilIdeal.Amarillo.gasRecomendado.min}-${perfilIdeal.Amarillo.gasRecomendado.max} bar) para esta fase.`,
        ...
        "Primer Crack": {
            ...
            soluciones: {
                ...
                gasBajo: (val) => `Gas Bajo: Tu ajuste de <strong>${val} bar</strong> está por debajo del rango recomendado (${perfilIdeal["Primer Crack"].gasRecomendado.min}-${perfilIdeal["Primer Crack"].gasRecomendado.max} bar) para esta fase.`,
                gasAlto: (val) => `Gas Alto: Tu ajuste de <strong>${val} bar</strong> está por encima del rango recomendado (${perfilIdeal["Primer Crack"].gasRecomendado.min}-${perfilIdeal["Primer Crack"].gasRecomendado.max} bar) para esta fase.`,
    ...
    function addEventListeners() {
        ...
        allDOMElements.gasSlider.addEventListener('input', (e) => { allDOMElements.gasValueDisplay.textContent = `${e.target.value} bar`; });
            ...
            if (!s.isTicking) {
                ...
                if (lastAdjustment) {
                    if (type === 'gas') {
                        ...
                        allDOMElements.gasValueDisplay.textContent = `${lastAdjustment.value} bar`;
                    } else {
    ...
    function verificarEstadoTueste(sample) {
        ...
        if (ultimoGas && perfilFase.gasRecomendado) {
            if (ultimoGas.value < perfilFase.gasRecomendado.min) mensajesParaMostrar.push({ tipo: 'warning', texto: `El gas actual (${ultimoGas.value} bar) parece bajo para esta fase (recomendado: ${perfilFase.gasRecomendado.min}-${perfilFase.gasRecomendado.max}).` });
            if (ultimoGas.value > perfilFase.gasRecomendado.max) mensajesParaMostrar.push({ tipo: 'warning', texto: `El gas actual (${ultimoGas.value} bar) parece alto para esta fase (recomendado: ${perfilFase.gasRecomendado.min}-${perfilFase.gasRecomendado.max}).` });
        }
        ...
        roastChart = new Chart(ctx, {
            ...
            data: {
                datasets: [
                    ...
                    { label: 'Gas (bar)', data: [], yAxisID: 'y', borderColor: 'hotpink', backgroundColor: 'rgba(255, 105, 180, 0.15)', fill: true, stepped: true },
        ...
        s.data.forEach((dataPoint) => {
            ...
            if (dataPoint.adjustment) {
                ...
                if (adj.type === 'gas') {
                    ...
                    tempCellContent += ` (Gas: ${adj.value} bar)`;
    ...
    async function generateSingleReport(id) { const { reportModal, reportBody, reportTitle, reportButtonsContainer } = allDOMElements; reportTitle.textContent = `Informe Individual - Muestra ${id}`; reportButtonsContainer.innerHTML = `<button id="print-report-btn-s">Imprimir</button><button id="export-pdf-btn-s" style="background-color: var(--color-d); color: white;">Exportar a PDF</button><button id="export-json-btn-s">Exportar a JSON</button><button id="export-csv-individual-btn-s">Exportar a CSV</button><button id="close-report-btn-s">Cerrar</button>`; document.getElementById('print-report-btn-s').onclick = () => window.print(); document.getElementById('close-report-btn-s').onclick = () => reportModal.classList.remove('visible'); document.getElementById('export-json-btn-s').onclick = () => exportProfileToJSON(id); document.getElementById('export-csv-individual-btn-s').onclick = () => exportSingleToCSV(id);
 document.getElementById('export-pdf-btn-s').onclick = () => exportReportToPDF(`Informe_Muestra_${id}`); reportBody.innerHTML = 'Generando...'; reportModal.classList.add('visible'); const sample = state.samples[id]; const { info, events, data, maxRoR } = sample; const asistenteAvisos = analizarPerfilCompleto(sample); const devTime = (events.crack && events.end) ? events.end.time - events.crack.time : 0; const filteredData = data.filter(d => d.temp !== null && !d.adjustment); const totalTime = events.end ? events.end.time : (filteredData.length > 0 ? filteredData[filteredData.length - 1].time : 0); const dtr = totalTime > 0 && devTime > 0 ? (devTime/totalTime*100).toFixed(1) : '0.0'; const loss = (info.greenWeight && info.roastedWeight) ? ((info.greenWeight-info.roastedWeight)/info.greenWeight*100).toFixed(2) : '0.00'; let logTableHtml = '<div class="report-log-container"><table class="report-table report-log-table"><thead><tr><th>Tiempo</th><th>Temp. (°C)</th><th>RoR (°C/min)</th></tr></thead><tbody>'; for (let i = 0; i < filteredData.length; i++) { const curr = filteredData[i]; let ror = '-'; if (i > 0) { const prev = filteredData[i-1]; const timeDiffMinutes = (curr.time - prev.time) / 60; if(timeDiffMinutes > 0) { ror = ((curr.temp - prev.temp) / timeDiffMinutes).toFixed(1); } } logTableHtml += `<tr><td>${formatTime(curr.time)}</td><td>${curr.temp.toFixed(1)}</td><td>${ror}</td></tr>`; } logTableHtml += '</tbody></table></div>'; let asistenteHtml = ''; if (asistenteAvisos && asistenteAvisos.length > 0) { asistenteHtml = `<div class="report-asistente-section"><h4>Observaciones del Asistente</h4>`; asistenteAvisos.forEach(aviso => { asistenteHtml += `<div class="aviso aviso-${aviso.tipoMensaje}">${aviso.texto}</div>`; }); asistenteHtml += `</div>`; } else { asistenteHtml = `<div class="report-asistente-section"><h4>Observaciones del Asistente</h4><p>No se registraron desviaciones significativas del perfil ideal durante este tueste.</p></div>`; } const img = await generateChartImageForReport(id, data, sample.adjustments); let equipoHtml = '';
    if (sample.equipo && sample.equipo.rol) {
        equipoHtml = `<tr><td colspan="2" style="background-color: var(--bg-dark); text-align: center; color: var(--text-color);"><strong>EQUIPO: ${sample.equipo.rol ? sample.equipo.rol.toUpperCase() : '-'}</strong></td></tr>
        <tr><td>Maestro Tostador</td><td>${sample.equipo.maestroTostador || '-'}</td></tr>`;
        if (sample.equipo.rol === 'Estudiante') {
            equipoHtml += `<tr><td>Tostador Adjunto</td><td>${sample.equipo.tostadorAdjunto || '-'}</td></tr>
            <tr><td>Reporte de Datos</td><td>${sample.equipo.reporteDatos || '-'}</td></tr>
            <tr><td>Registro Físico</td><td>${sample.equipo.registroFisico || '-'}</td></tr>
            <tr><td>Registro Plataforma</td><td>${sample.equipo.registroPlataforma || '-'}</td></tr>`;
        }
    }
    const html = `<section class="report-single-section" style="border-left: 5px solid ${getSampleColor(id)}"><table class="report-table">${equipoHtml}<tr><td colspan="2" style="background-color: var(--bg-dark); text-align: center; color: var(--text-color);"><strong>DATOS DEL LOTE</strong></td></tr><tr><td>Variedad</td><td>${info.variety||'-'}</td></tr><tr><td>Productor</td><td>${info.producer||'-'}</td></tr><tr><td>Beneficio</td><td>${info.process||'-'}</td></tr><tr><td>Peso Almendra</td><td>${info.greenWeight||'-'} g</td></tr><tr><td>Humedad Almendra</td><td>${info.greenMoisture||'-'} %</td></tr><tr><td>Densidad</td><td>${info.density||'-'} g/ml</td></tr><tr><td>Peso Tostado</td><td>${info.roastedWeight||'-'} g</td></tr><tr><td><strong>Pérdida de Peso</strong></td><td><strong>${loss} %</strong></td></tr><tr><td>Tiempo Total</td><td>${formatTime(totalTime)}</td></tr><tr><td>Tiempo Desarrollo</td><td>${formatTime(devTime>0?devTime:0)}</td></tr><tr><td><strong>Ratio Desarrollo (DTR)</strong></td><td><strong>${dtr} %</strong></td></tr><tr><td><strong>RoR Máximo</strong></td><td><strong>${maxRoR.value > -Infinity ? maxRoR.value.toFixed(1) : '-'}</strong></td></tr></table><div class="chart-container"><img src="${img}" alt="Gráfica Muestra ${id}" class="report-single-graph"></div>${asistenteHtml}${logTableHtml}</section>`; reportBody.innerHTML = html; }
    async function generateConsolidatedReport() { const { reportModal, reportBody, reportTitle, reportButtonsContainer } = allDOMElements; reportTitle.textContent = 'Informe de Tueste Consolidado'; reportButtonsContainer.innerHTML = `<button id="print-report-btn-c">Imprimir</button><button id="export-pdf-btn-c" style="background-color: var(--color-d); color: white;">Exportar a PDF</button><button id="export-csv-consolidated-btn-c">Exportar Todo a CSV</button><button id="close-report-btn-c">Cerrar</button>`; document.getElementById('print-report-btn-c').onclick = () => window.print(); document.getElementById('close-report-btn-c').onclick = () => reportModal.classList.remove('visible'); document.getElementById('export-csv-consolidated-btn-c').onclick = () => exportConsolidatedToCSV();
 document.getElementById('export-pdf-btn-c').onclick = () => exportReportToPDF('Informe_Consolidado_Tuestes'); reportBody.innerHTML = 'Generando...'; reportModal.classList.add('visible'); const samplesWithData = Object.keys(state.samples).filter(id => state.samples[id].data.filter(d => d.temp !== null).length > 0); if (samplesWithData.length === 0) { reportBody.innerHTML = '<p>No hay tuestes para reportar.</p>'; return; } const tableHeaders = samplesWithData.map(id => `<th style="background-color:${getSampleColor(id)}; color:white;">MUESTRA ${id}</th>`).join(''); const infoFields = [ { label: 'Variedad', key: 'variety' }, { label: 'Productor', key: 'producer' }, { label: 'Beneficio', key: 'process' }, { label: 'Peso Almendra (g)', key: 'greenWeight' }, { label: 'Humedad Almendra (%)', key: 'greenMoisture' }, { label: 'Densidad (g/ml)', key: 'density' }, { label: 'Peso Tostado (g)', key: 'roastedWeight' } ]; 
    const teamFields = [
        { label: 'Rol', key: 'rol' },
        { label: 'Maestro Tostador', key: 'maestroTostador' },
        { label: 'Tostador Adjunto', key: 'tostadorAdjunto' },
        { label: 'Reporte de datos', key: 'reporteDatos' },
        { label: 'Registro de datos físicos', key: 'registroFisico' },
        { label: 'Registro Plataforma', key: 'registroPlataforma' }
    ];
    let teamRows = teamFields.map(field => `<tr><td><strong>${field.label}</strong></td>${samplesWithData.map(id => {
        const eq = state.samples[id].equipo;
        let val = eq ? eq[field.key] : '-';
        if (!val) val = '-';
        return `<td class="value-cell">${val}</td>`;
    }).join('')}</tr>`).join('');
    
    let summaryRows = `<tr><td colspan="${samplesWithData.length + 1}" style="background-color: var(--bg-dark); text-align: center; color: var(--text-color);"><strong>DATOS DEL EQUIPO</strong></td></tr>` + teamRows + `<tr><td colspan="${samplesWithData.length + 1}" style="background-color: var(--bg-dark); text-align: center; color: var(--text-color);"><strong>DATOS DEL LOTE</strong></td></tr>` + infoFields.map(field => `<tr><td>${field.label}</td>${samplesWithData.map(id => `<td class="value-cell">${state.samples[id].info[field.key] || '-'}</td>`).join('')}</tr>`).join('');
 const lossRow = `<tr><td><strong>Pérdida de Peso (%)</strong></td>${samplesWithData.map(id => { const {greenWeight, roastedWeight} = state.samples[id].info; return `<td class="value-cell">${(greenWeight > 0 && roastedWeight > 0) ? ((greenWeight - roastedWeight) / greenWeight * 100).toFixed(2) : '-'}</td>`}).join('')}</tr>`; const devTimeRow = `<tr><td>Tiempo Desarrollo</td>${samplesWithData.map(id => { const {crack, end} = state.samples[id].events; const devTime = (crack && end) ? end.time - crack.time : 0; return `<td class="value-cell">${formatTime(devTime > 0 ? devTime : 0)}</td>`}).join('')}</tr>`; const dtrRow = `<tr><td><strong>Ratio Desarrollo (DTR) (%)</strong></td>${samplesWithData.map(id => { const {crack, end} = state.samples[id].events; const devTime = (crack && end) ? end.time - crack.time : 0; const totalTime = end ? end.time : 0; return `<td class="value-cell">${(totalTime > 0 && devTime > 0) ? (devTime / totalTime * 100).toFixed(1) : '-'}</td>`}).join('')}</tr>`; summaryRows += lossRow + devTimeRow + dtrRow; let eventRows = eventOrder.map(key => `<tr><td>${eventNames[key]}</td>${samplesWithData.map(id => { const event = state.samples[id].events[key]; return `<td class="value-cell">${event ? `${formatTime(event.time)} - ${event.temp.toFixed(1)}°C` : '-'}</td>`; }).join('')}</tr>`).join(''); let maxRoRRow = `<tr><td><strong>RoR Máximo</strong></td>${samplesWithData.map(id => `<td class="value-cell">${state.samples[id].maxRoR.value > -Infinity ? state.samples[id].maxRoR.value.toFixed(1) : '-'}</td>`).join('')}</tr>`; const summaryTable = `<div class="print-page"><table class="report-table"><thead><tr><th>DATOS Y RESULTADOS</th>${tableHeaders}</tr></thead><tbody>${summaryRows}${eventRows}${maxRoRRow}</tbody></table></div>`; let maxTime = 0; samplesWithData.forEach(id => { const s = state.samples[id]; const lastDataPoint = s.data.filter(d=>d.temp!==null).pop(); if (lastDataPoint && lastDataPoint.time > maxTime) maxTime = lastDataPoint.time; }); let logRows = ''; for(let t = 0; t <= maxTime; t += LOG_INTERVAL) { let row = `<tr><td>${formatTime(t)}</td>`; samplesWithData.forEach(id => { const dataPoint = state.samples[id].data.find(d => d.time === t); row += `<td class="value-cell">${dataPoint && dataPoint.temp !== null ? dataPoint.temp.toFixed(1) : ''}</td>`; }); row += '</tr>'; logRows += row; } const logTable = `<div class="print-page"><div class="report-log-container"><table class="report-table report-log-table"><thead><tr><th>TIEMPO</th>${tableHeaders}</tr></thead><tbody>${logRows}</tbody></table></div></div>`; const graphPage = `<div class="print-page print-graph-page"><div class="report-graph-container"><h3>GRÁFICA DEL PERFIL DE TUESTE</h3><div class="chart-container"><canvas id="consolidated-report-chart"></canvas></div></div></div>`; reportBody.innerHTML = `<div class="report-container">${summaryTable}${logTable}${graphPage}</div>`; const datasets = samplesWithData.map(id => { const s = state.samples[id]; const dataArray = s.data.filter(d => d.temp !== null && !d.adjustment).sort((a,b) => a.time - b.time); const eventTimeToIcon = {}; Object.keys(s.events).forEach(key => { const event = s.events[key]; if(event && event.time !== undefined) { eventTimeToIcon[event.time] = pointIcons[key] || 'circle'; } }); return { label: `Muestra ${id}`, data: dataArray.map(d => ({ x: d.time * 1000, y: d.temp })), borderColor: getSampleColor(id), tension: 0.1, fill: false, pointRadius: dataArray.map(d => eventTimeToIcon[d.time] ? 8 : 3), pointStyle: dataArray.map(d => eventTimeToIcon[d.time] || 'circle'), pointBackgroundColor: getSampleColor(id) }; }); const ctx = document.getElementById('consolidated-report-chart').getContext('2d'); new Chart(ctx, { type: 'line', data: { datasets }, options: { responsive: true, maintainAspectRatio: false, animation: false, scales: { x: { type: 'time', time: { parser: 'mm:ss', unit: 'minute', displayFormats: { minute: 'mm:ss' } }, title: { display: true, text: 'Tiempo' } }, y: { suggestedMin: 100, suggestedMax: 240, title: { display: true, text: 'Temperatura (°C)'} } } } }); }
            ...
            new Chart(ctx, {
                ...
                data: {
                    datasets: [
                        ...
                        { label: 'Gas (bar)', data: gasData, borderColor: 'hotpink', backgroundColor: 'rgba(255, 105, 180, 0.15)', yAxisID: 'y', stepped: true, fill: true },
    ...
    function exportReportToPDF(filename) {
        ...
        // Preparar el elemento para el PDF (quitar scrolls temporales si los hay)
        ...
        // Cambiar el texto de los botones mientras carga
        const btns = document.querySelectorAll('#export-pdf-btn-s, #export-pdf-btn-c');
        btns.forEach(b => { b.textContent = "Generando..."; b.disabled = true; });

        html2pdf().set(opt).from(element).save().then(() => {
            // Restaurar estilos
            element.style.height = originalHeight;
            element.style.overflow = originalOverflow;
            btns.forEach(b => { b.textContent = "Exportar a PDF"; b.disabled = false; });
        }).catch(err => {
    ...
    if (saveCloudBtn) {
        saveCloudBtn.addEventListener('click', () => {
            ...
            const payload = {
                ...
                equipo: s.equipo || window.sessionRoles,
                rawSample: s
            };
        ...
        loadCloudBtn.addEventListener('click', () => {
            ...
            .then(data => {
                if (data.status === "éxito") {
                    ...
                    tableHTML += '<thead><tr><th>Fila</th><th>Fecha</th><th>Muestra</th><th>Rol</th><th>Maestro Tostador</th><th>Acción</th></tr></thead><tbody>';
                    ...
                    data.list.forEach(item => {
                        tableHTML += `<tr>
                            ...
                            <td style="text-align:center; padding: 6px;">${item.rol || '-'}</td>
                            <td style="text-align:center; padding: 6px;">${item.maestro || '-'}</td>
...
try { document.getElementById('gas-value-display').textContent = '60 bar'; } catch(e){}
